...
- Table reads inside an `enforce` condition: REPL-invisible, FAIL on upstream-lineage nodes (allowed on KDA-CE 3.1+) — defensive default is `let`-binding before the `enforce`; flag unbound reads on portable/public code (severity INFO on pinned KDA-CE 3.1+ targets). Rule: pact-traps §Read-only context
